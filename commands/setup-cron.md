...
Default cadence: once per day. This installs one all-pairs sweep job that runs
`/repost-run all`. Do not create staggered per-pair cron jobs unless Ethan
...
- [ ] At least one pair has `mode === "live-approved"`
- [ ] At least one pair has `runMode === "listen-for-future"`
- [ ] User is logged into source + destination platforms needed by enabled live-approved pairs in the current harness browser profile
...
- **OpenClaw workflows (preferred):** one `openclaw cron` job that starts a
  fresh isolated agent turn with `--message "/repost-run all"` and
  `thinking=medium` by default. The scheduled agent sweeps pairs sequentially in
  that single turn. Escalate only when the user or pair explicitly needs heavier
  reasoning.
...
- OpenClaw: `openclaw cron show repost-with-agent.all.daily` to confirm the exact job,
...
- `/pair edit <pair-id>` — adjust pair eligibility/policy; cadence is global unless Ethan explicitly requests per-pair scheduling.
